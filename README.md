...
*Примечание по LLM-as-a-Judge: в коде используется Mistral API. Если вы будете использовать другого провайдера, например OpenRouter, можно отправлять запросы к RAG и Judge LLM одновременно, что сэкономит время на проверку (Mistral API ограничен 1 запросом в секунду и 500 000 токенами в минуту). Для этого нужно раскомментировать строки в `evaluate_all_questions`*
